...
# ---------------------------------------------------
# ⚠️ NEW: SAFE TEXT EXTRACTOR (TXT ONLY)
# ---------------------------------------------------
def extract_text(file):
    """Extract text from .txt only (PDF removed for Streamlit Cloud safety)."""
    if file.name.endswith(".txt"):
        return file.read().decode("utf-8")
    else:
        return "PDF upload is not supported on Streamlit Cloud. Please upload .txt version."
...
uploaded_files = st.file_uploader(
    "Upload internal documents (.txt only)",
    type=["txt"],
